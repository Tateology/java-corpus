 ...
 * freecs.commands
 */
public class CmdUnlock extends AbstractCommand {
	private final String cmd= "/ul";
	private final String version = "1.0";
	private static final ICommand selve=new CmdUnlock();
	...
	public boolean execute (MessageState msgState, String param) {
        ...
        if (msgState.targetGroup.usrMayLock(msgState.sender)) {
            msgState.targetGroup.setState (IGroupState.OPEN);
            msgState.msgTemplate = "message.ul";
        ...
        } else if (msgState.targetGroup.hasState (IGroupState.LOCKPROTECTED)) {
            msgState.msgTemplate = "error.ul.noRight";
            msgState.sender.sendMessage (msgState.mp);
        } else if (msgState.targetGroup.hasState (IGroupState.MODERATED)) {
