 ...
 * freecs.commands
 */
public class CmdLock extends AbstractCommand {
	private final String cmd= "/l";
	private final String version = "1.0";
	private static final ICommand selve=new CmdLock();
	...
	public boolean execute (MessageState msgState, String param) {
		...
		if (!msgState.targetGroup.hasState (IGroupState.OPEN)) 
			return false;
		if (msgState.targetGroup.hasState(IGroupState.LOCKPROTECTED)) {
            msgState.msgTemplate = "error.noRight.deactivated";
            msgState.sender.sendMessage (msgState.mp);
            return false;
        } else if (!msgState.targetGroup.usrMayLock(msgState.sender)) {
		...
		msgState.msgTemplate = "message.l";
