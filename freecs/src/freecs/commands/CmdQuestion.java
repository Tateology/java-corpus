...
public class CmdQuestion extends AbstractCommand {
	private final String cmd= "/aq";
	private final String version = "1.0";
	private static final ICommand selve=new CmdQuestion();
	...
	public boolean execute (MessageState msgState, String param) {
		if (isPunished (msgState)) 
			return false;
		if (param.length () < 1) {
		   msgState.msgTemplate = "error.aq.noArg";
		   msgState.sender.sendMessage (msgState.mp);
		   return false;
		}
        msgState.targetGroup = msgState.sender.getGroup ();
        synchronized (msgState.targetGroup) {
            msgState.targetGroup.incrementQuestionCounter ();
    		msgState.sender.incrementQuestionCounter ();
    		msgState.message = param;
    		msgState.msgTemplate = "message.aq";
