 ...
 * freecs.commands
 */
public class CmdShout extends AbstractCommand {
	private final String cmd= "/s";
	private final String version = "1.0";
	private static final ICommand selve=new CmdShout();
	...
	public boolean execute (MessageState msgState, String param) {
		if (isPunished (msgState)) 
			return false;
		if (param.length () < 1) {
		   msgState.msgTemplate = "error.s.noArg";
		   msgState.sender.sendMessage (msgState.mp);
		   return false;
		}
		msgState.msgTemplate = "message.s";
