		...
		mockery.checking(new Expectations() {
			{
				...
				one(monitor).invoked(with(aNull(PicoContainer.class)),
                        ...
                        with(any(Long.class)), with(same((Float)null)), with(any(Object[].class)));
			...
			mockery.checking(new Expectations() {
				{
					...
					one(monitor).invoked(
							with(aNull(PicoContainer.class)),
							with(aNull(ComponentAdapter.class)),
							with(isStopMethod), with(same(mock)), with(any(Long.class)), with(same((Float)null)), with(any(Object[].class)));
				}
			});
			return mock;
		}
