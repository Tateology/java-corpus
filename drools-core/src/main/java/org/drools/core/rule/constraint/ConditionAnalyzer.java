...
public class ConditionAnalyzer {
    ...
    private Condition analyzeCondition(ASTNode node) {
        ...
        if (node instanceof Negation) {
            isNegated = true;
            ExecutableStatement statement = null; //((Negation)node).getStatement();
        ...
        if (node instanceof Negation) {
            isNegated = !isNegated;
            node = null; //((ExecutableAccessor)((Negation)node).getStatement()).getNode();
            node = analyzeSubstatement(node);
        }
    ...
    private SingleCondition analyzeSingleCondition(ASTNode node, boolean isNegated) {
        ...
        if (node instanceof BinaryOperation) {
            ...
            Pattern pattern = null; //regExNode.getPattern();
            if (pattern != null) {
                condition.right = new FixedExpression(String.class, pattern.pattern());
            } else {
                condition.right = null; //analyzeNode(((ExecutableAccessor)regExNode.getPatternStatement()).getNode());
            }
        } else if (node instanceof Contains) {
            condition.left = null; //analyzeNode(((Contains)node).getFirstStatement());
            condition.operation = BooleanOperator.CONTAINS;
            condition.right = null; //analyzeNode(((Contains)node).getSecondStatement());
        } else if (node instanceof Soundslike) {
            condition.left = null; //analyzeNode(((Soundslike) node).getStatement());
            condition.operation = BooleanOperator.SOUNDSLIKE;
            condition.right = null;//analyzeNode(((Soundslike)node).getSoundslike());
        } else if (node instanceof Instance) {
            condition.left = null; //analyzeNode(((Instance) node).getStatement());
            condition.operation = BooleanOperator.INSTANCEOF;
            condition.right = null; //analyzeNode(((Instance) node).getClassStatement());
        } else {
            condition.left = analyzeNode(node);
        }
        return condition;
    ...
    private ASTNode analyzeRegEx(ASTNode node) {
        if (node instanceof RegExMatch) {
            return null; //((ExecutableAccessor)((RegExMatch)node).getStatement()).getNode();
        }
        return node;
    }
    ...
    private Expression analyzeNode(ASTNode node) {
        ...
        if (node instanceof TypeCast) {
            ExecutableStatement statement = null; //((TypeCast)node).getStatement();
        ...
        if (node instanceof Union) {
            ASTNode main = null; //((Union)node).getMain();
            ...
            if (accessor instanceof DynamicGetAccessor) {
                AccessorNode accessorNode = null; //(AccessorNode)((DynamicGetAccessor)accessor).getSafeAccessor();
    ...
    private Expression analyzeNodeAccessor(Accessor accessor, ASTNode node) {
        ...
        if (accessor instanceof DynamicGetAccessor) {
            accessorNode = null; //(AccessorNode)((DynamicGetAccessor)accessor).getSafeAccessor();
        } else if (accessor instanceof AccessorNode) {
    ...
    private Invocation analyzeAccessorInvocation(AccessorNode accessorNode, ASTNode containingNode, Invocation formerInvocation, Class<?> variableType) {
        ...
        if (accessorNode instanceof ConstructorAccessor) {
            ...
            Constructor constructor = null; //constructorAccessor.getConstructor();
        ...
        if (accessorNode instanceof StaticVarAccessor) {
            Field field = null; //((StaticVarAccessor)accessorNode).getField();
