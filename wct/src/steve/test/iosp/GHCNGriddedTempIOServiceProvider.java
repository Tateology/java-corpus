 ...
 *
 */
public class GHCNGriddedTempIOServiceProvider extends AbstractIOServiceProvider {
	...
	public boolean isValidFile(RandomAccessFile raf) throws IOException {
		...
		if (filename.equals("grid_1880-2009_RVose.dat") || filename.equals("ncdc_blended_merg53v3b.dat")) {
			return (raf.readLine().trim().equals("1  1880"));			
		}
		else {
			return false;
		}
	...
	public void open(RandomAccessFile raf, NetcdfFile ncfile, CancelTask cancelTask) throws IOException {
		...
		String path = raf.getLocation();
		String filename = path.substring(path.lastIndexOf("/")+1);
		int numMonths = (filename.equals("grid_1880-2009_RVose.dat")) ? 1560 : 1572;
		
				
		Array dataArray = Array.factory(DataType.INT, new int[]{ numMonths, 36, 72});
		Index dataIndex = dataArray.getIndex();
		Array timeArray = Array.factory(DataType.INT, new int[]{ numMonths });
		...
		while (! raf.isAtEndOfFile() && raf != null) {
			...
			for (int y=0; y<36; y++) {
				...
				if (cols.length != 72) {
					throw new IOException("72 data columns - not found... - found "+cols.length+"\n"+str);
				}
				for (int c=0; c<cols.length; c++) {
					dataArray.setInt(dataIndex.set(t, y, c), Integer.parseInt(cols[c]));
				}
			}
			try {
//				System.out.println(sdf.parse(dateString.trim()).getTime()/1000/60/60/24);
		...
		ncfile.addDimension(null, new Dimension("time", numMonths));
		ncfile.addDimension(null, new Dimension("lat", 36));
		ncfile.addDimension(null, new Dimension("lon", 72));
		
		Variable v = new Variable(ncfile, null, null, "temperature", DataType.INT, "time lat lon");
		v.addAttribute(new Attribute("scale_factor", 0.01));
		v.addAttribute(new Attribute("add_offset", 0));
		v.addAttribute(new Attribute("_FillValue", -9999));
		v.addAttribute(new Attribute("units", "degrees_celsius"));
		v.addAttribute(new Attribute("standard_name", "air_temperature_anomaly"));
	...
	public static void main(String[] args) {
		try {
			NetcdfFile.registerIOProvider(GHCNGriddedTempIOServiceProvider.class);
			String infile = "E:\\work\\station-data\\ncdc_blended_merg53v3b.dat";
//			String infile = "E:\\work\\station-data\\grid_1880-2009_RVose.dat";
			NetcdfFile ncfile=NetcdfFile.open(infile);
			FileWriter.writeToFile(ncfile, "E:\\work\\station-data\\ncdc_blended_merg53v3b.nc").close();
//			FileWriter.writeToFile(ncfile, "E:\\work\\station-data\\grid_1880-2009_RVose.nc").close();
