...
import gov.noaa.ncdc.iosp.area.AreaIo;
...
public class NPPIosp extends AbstractIOServiceProvider {


	private List<Variable> varList;
	private List<Attribute> globalAttList;
	...
	private NetcdfFile h5file = null;
	...
	public boolean isValidFile(RandomAccessFile raf) throws IOException {
		...
		System.out.println("in isValidFile in NPPIosp");

		NetcdfFile ncfile = null;
		try {
			ncfile = NetcdfFile.open(raf.getLocation(), "ucar.nc2.iosp.hdf5.H5iosp", 1024*10, null, "");
	...
	throws IOException	{
		...
		try {
			h5file = NetcdfFile.open(raf.getLocation(), "ucar.nc2.iosp.hdf5.H5iosp", 1024*10, null, "");
			
			List<Variable> varList = loadHd5(h5file);
			buildNetCdf(ncfile, new Vector<NppProfile>(), varList);
		} catch (Exception e) {
			e.printStackTrace();
			if (h5file != null) h5file.close();
...
//			buildNetCdf(ncfile, new Vector<NppProfile>(), varList);
			...
			for (Variable v : h5file.getVariables()) {
			
				if (v.getShortName().equalsIgnoreCase((v2.getShortName()))) {
					data = v.read(section);
				}				
			}
	 ...
	 * @throws Exception
	 * *************************************************************************************************************/
	private List<Variable> loadHd5(NetcdfFile ncH5) throws Exception
	{
		int iGeoX = -9, iGeoY = -9;
...
//					varList.get(x).setName(varList.get(y).getShortName().trim() + "_2";
					varList.get(x).setName(varList.get(y).getParentGroup().getName() + "/"+varList.get(y).getShortName().trim());
				}
	 ...
	 * ***************************************************************************************************************/
	private void buildNetCdf(NetcdfFile ncFile,
	...
	/*****************************************************************************************************************/
	private void addProfileAtts(NetcdfFile ncFile, Variable var,
			NppProfile profile)
	{
	...
	/*****************************************************************************************************************/
	private void cfAtts(NetcdfFile ncFile, NppProfile nppProf, int iProfileFieldNumber, 
	...
	/*****************************************************************************************************************/
	private void addVarAttributes(NetcdfFile ncFile, Variable var)
	{
		
		Variable newVar = ncFile.findVariable(var.getShortName().toLowerCase().trim());
		...
		for(int x = 0; x < var.getAttributes().size(); x++) // Add Variable Attributes
		{
			...
			if(att.getDataType().toString().equalsIgnoreCase("LONG")) // Long is invalid type - Change to double
				ncFile.addVariableAttribute(newVar, new Attribute(att.getName(), att.getNumericValue().doubleValue()));
			else
				if(att.isString())
					ncFile.addVariableAttribute(newVar, new Attribute(att.getName(), att.getStringValue()));
				else
					ncFile.addVariableAttribute(newVar, new Attribute(att.getName(), att.getNumericValue()));
		}

		// Dataset specific atts
		if(var.getShortName().equalsIgnoreCase("lat"))
		{
			ncFile.addVariableAttribute(newVar, new Attribute("units", "degrees_north"));
			ncFile.addVariableAttribute(newVar, new Attribute("long_name", "latitude"));
			ncFile.addVariableAttribute(newVar, new Attribute("standard_name", "latitude"));
		}
		else
			if(var.getShortName().equalsIgnoreCase("lon"))
			{
				ncFile.addVariableAttribute(newVar, new Attribute("units", "degrees_east"));
				ncFile.addVariableAttribute(newVar, new Attribute("long_name", "longitude"));
				ncFile.addVariableAttribute(newVar, new Attribute("standard_name", "longitude"));
			}
			else
		...
		if(isSpecDs(var)) // If this is a spec DS add Coordinates Att
			ncFile.addVariableAttribute(newVar, new Attribute("coordinates", "lat lon"));
	}
