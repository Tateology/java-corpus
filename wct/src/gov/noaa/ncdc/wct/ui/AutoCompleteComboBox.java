...
public class AutoCompleteComboBox<E> extends JComboBox {
    ...
    public AutoCompleteComboBox(Object[] objs) {
    	super(objs);
        AutoCompleteComboBoxEditor cbe = new AutoCompleteComboBoxEditor();
        this.editorComp = (AutoCompleteTextField)(cbe.getEditorComponent());
        this.setEditor(cbe);
        
//        ((BasicComboBoxRenderer)this.getRenderer()).setBorder(BorderFactory.createEmptyBorder(0,13,0,13));
//      this.editorComp.setMargin(new Insets(0, 10, 0, 0));
      this.editorComp.setBorder(BorderFactory.createCompoundBorder(
...
//        this.incompleteColor = Color.GRAY.brighter();
        this.incompleteColor = new Color(185, 153, 128);
        this.currentGuess = -1;
...
//            if (possibility.startsWith(entered)) {
            if (possibility.contains(entered)) {
                this.currentGuess = i;
                break;
            }
        }
...
//            g.drawString(subGuess + "   press ENTER to send or right arrow to fill", (int)(enteredBounds.getWidth()) + 2, centeredY - 2);
//        }
        ...
        if (!(guess.contains(entered)))
            this.areGuessing = false;

        if (entered != null && !(entered.equals("")) 
                && this.areGuessing) {
            String subGuess = drawGuess;
            Rectangle2D subGuessBounds = g.getFontMetrics().getStringBounds(drawGuess, g);
            ...
            g.setColor(this.incompleteColor);
//            g.drawString(subGuess + "   press ENTER to send or right arrow to fill", (int)(enteredBounds.getWidth()) + 2, centeredY - 2);
            g.drawString(subGuess + "   [press ENTER to fill]", (int)(enteredBounds.getWidth()) + 18, centeredY - 2);
        }
