 ...
 * @created    Feb 15, 2005
 */
public class DecodeNCRasterHeader implements NexradHeader {
   ...
   public DecodeNCRasterHeader() {
   }
   
   //---------------------------------------------------------------
   // Empty implementations of NexradHeader
   ...
   public void decodeHeader(URL url) {
      ...
      try {
         ...
         ncFile = NetcdfFile.open(url.toString());
         ...
         Variable precip = ncFile.findVariable("Total_precipitation");
         List dims = precip.getDimensions();
         for (int n = 0; n < dims.size(); n++) {
            System.out.println(dims.get(n).toString());
         }

         System.out.println("\n\nNCEP GRIB STAGE-4 DATA: \n");
         Array data = precip.read();
...
*/
   ...
   public double[] getLLCorner() throws IOException {

      Variable lat = ncFile.findVariable("lat");
      Variable lon = ncFile.findVariable("lon");
      Array latdata = lat.read();
      Array londata = lon.read();
      double llLat = latdata.getDouble(latdata.getIndex().set(0));
      double llLon = londata.getDouble(londata.getIndex().set(0));

      return new double[] { llLon, llLon };
   }
   ...
   public String getICAO() {
      System.err.println("getICAO method not supported for DecodeNCRasterHeader  "+ getDataURL());
      return null;
   }
   ...
   public void finalize() {
      try {
         System.out.println("CALLING DecodeNCRasterHeader FINALIZE");
         ncFile.close();
      } catch (Exception e) {
         e.printStackTrace();
      }
   ...
   public static void main(String[] args) {
      try {
         DecodeNCRasterHeader xmrgHeader = new DecodeNCRasterHeader();
         //xmrgHeader.decodeHeader(new java.io.File("D:\\Nexrad_Viewer_Test\\XMRG\\serfc\\xmrg_09192004_00z_SE").toURL());
         
         //URL url = new URL("ftp://ftp.joss.ucar.edu/pub/download/codiac/steve10014/ST4.2006020100.01h.Z");
         //URL url = new URL("http://www1/pub/download/ansari/ST4.2006030112.24h.Z");
         URL url = new URL("ftp://ftp0/pub/download/ansari/ST4.2006030112.24h.Z");
         
         url = Level2Transfer.getNCDCLevel2UNIXZ(url, true, null);         
         xmrgHeader.decodeHeader(url);
      } catch (Exception e) {
         e.printStackTrace();
      }
   }
