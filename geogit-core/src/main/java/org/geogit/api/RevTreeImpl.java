...
public abstract class RevTreeImpl extends AbstractRevObject implements RevTree {
    ...
    public static RevTreeImpl createLeafTree(ObjectId id, long size, Collection<Node> features,
        ...
        if (!features.isEmpty()) {
            featuresList = null; //ordering.immutableSortedCopy(features);
        }
        if (!trees.isEmpty()) {
            treesList = null; //ordering.immutableSortedCopy(trees);
        }
    ...
    public Iterator<Node> children() {
        ...
        return null; // Iterators.mergeSorted(ImmutableList.of(trees.iterator(), features.iterator()),
                // ordering);
    }
