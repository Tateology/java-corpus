 ...
 **************************************************************************/
    ...
    Type checkCastable(DiagnosticPosition pos, Type found, Type req) {
    	// blg59
//    	String msg = String.format("<Check.checkCastable>%s ==> %s",
//    			found.toString(), req.toString());
//    	log.note(pos, "proc.messager", msg);
    	//-blg59
        if (found.tag == FORALL) {
...
//where
    ...
    public boolean checkValidGenericType(Type t) {
    	//blg59 wtf this is never called
        return firstIncompatibleTypeArg(t, -9001) == null;
    }
    //WHERE
        private Type firstIncompatibleTypeArg(Type type, int pos) {
            ...
            while (args.nonEmpty() && tvars.nonEmpty()) {
                ...
                if (!isTypeArgErroneous(actual) &&
                	...
                	// blg59 log here
                	TypeVar bs = (TypeVar)tvars.head;
//                	if (pos != -9001){
//                		String msg = String.format("<Check.checkExtends>%s ==> %s {%s - %s}",
//            			    actual, bs, bs.getLowerBound(), bs.getUpperBound());
//                		log.note(pos, "proc.messager", msg);
//                	}
                	if (!checkExtends(actual, bs)) {
                		return args.head;
                	}
 ...
 **************************************************************************/
    ...
    void validate(JCTree tree, Env<AttrContext> env, Shapes.SHAPE setting) {
        validate(tree, env, true, setting);
    }
    void validate(JCTree tree, Env<AttrContext> env, boolean checkRaw, Shapes.SHAPE setting) {
    	// blg59: pass on setting, handled in the visitor
    	new Validator(env).validateTree(tree, checkRaw, true, setting);
    }

    /** Visitor method: Validate a list of type expressions.
     */
    void validate(List<? extends JCTree> trees, Env<AttrContext> env, Shapes.SHAPE setting) {
    	// blg59: same setting passed on to each element
    	for (List<? extends JCTree> l = trees; l.nonEmpty(); l = l.tail)
            validate(l.head, env, setting);
    }
    
    /**
     * blg59: affirm that tree conforms to shape setting
     */
//    void validateShape(JCTree tree, Env<AttrContext> env, Shapes.SHAPE setting) {
//    	if (setting = Shapes.SHAPE.NOTOK && Shapes.isShape(tree.type)) {
//    		log.note(tree.pos, "proc.messager", "SHAPE ABUSE");
//    	}
//    	switch (setting) {
//    	case OK:
////    		log.note(tree.pos, "proc.messager", "SHAPEOK");
//    		break;
//    	case NOTOK:
//    		if (Shapes.isShape(tree.type)) {
//        		log.note(tree.pos, "proc.messager", "SHAPE ABUSE");
//    		}
//    		break;
////    	case TOPLEVEL:
////    		// Shapes are only allowed at the top level
////    		// have tree accept a visitor?
////    		System.out.printf("Nested\n");
////    		if (tree instanceof JCTree.JCTypeApply) {
////    			System.out.printf("%s is a type apply\n", tree);
////    			for (JCTree.JCExpression jce : ((JCTree.JCTypeApply)tree).getTypeArguments()) {
////    				// Make sure type arguments are not shapes. 
////    				validateShape((JCTree)jce, env, Shapes.SHAPE.NOTOK);
////    				validateShape((JCTree)jce, env, Shapes.SHAPE.TOPLEVEL);
////    			}
////    		}
////    		break;
//    	default: 
    ...
    class Validator extends JCTree.Visitor {
        ...
        public void visitTypeApply(JCTypeApply tree) {
            if (tree.type.tag == CLASS) {
                ...
                Type incompatibleArg = firstIncompatibleTypeArg(tree.type, tree.pos);
        ...
        public void visitTypeParameter(JCTypeParameter tree) {
            validateTrees(tree.bounds, true, isOuter, Shapes.SHAPE.OK);
        ...
        public void visitWildcard(JCWildcard tree) {
            if (tree.inner != null)
                validateTree(tree.inner, true, isOuter, Shapes.SHAPE.NOTOK);
        }
        ...
        public void validateShape(JCTree tree, Shapes.SHAPE setting) {
        	if (setting == Shapes.SHAPE.NOTOK && Shapes.isShape(tree.type)) {
          // TODO 2013-11-13
        		log.note(tree.pos, "shapeabuse");
        		//log.warning(tree.pos, "shapeabuse");
        	}
        }
        ...
        public void validateTree(JCTree tree, boolean checkRaw, boolean isOuter, Shapes.SHAPE setting) {
            try {
                if (tree != null) {
                	//blg59
                	validateShape(tree, setting);
        ...
        public void validateTrees(List<? extends JCTree> trees, boolean checkRaw, boolean isOuter, Shapes.SHAPE setting) {
            for (List<? extends JCTree> l = trees; l.nonEmpty(); l = l.tail)
                validateTree(l.head, checkRaw, isOuter, setting);
        }
...
//where
    ...
    class CycleChecker extends TreeScanner {
        ...
        void checkClass(DiagnosticPosition pos, Symbol c, List<JCTree> supertypes) {
        	// blg59: begin diagnostics
        	// 2013-08-06: Maybe not necessary. maybe taken care of by checkcyclic
//        	String superStr = "";
//            for (JCTree sType : supertypes) {
//            	if (sType != null && sType.type != null){
//            		superStr += sType.type.toString() + "; ";
//            	}
//            }
//            if (superStr != "") {
//            	String msg = String.format("<Check.checkClass:supertypes>%s ==> [%s]", 
//            		c, superStr.substring(0, superStr.length()-2));
//            	log.note(pos, "proc.messager", msg);
//            }
            //WEPA
...
//        	String sceneStr = "";
//        	for (TypeVar sType : seen) {
//        		sceneStr += String.format("{%s - %s}; ",
//        				sType.getLowerBound(), sType.getUpperBound()); 
//        	}
//        	
//        	if (!sceneStr.equals("")) {
//        		String msg = String.format("<Check.checkNonCyclicl>%s ==> %s",
//        			t, sceneStr.substring(0, sceneStr.length()-2));
//        		log.note(pos, "proc.messager", msg);
//        	}
            return;
        }
