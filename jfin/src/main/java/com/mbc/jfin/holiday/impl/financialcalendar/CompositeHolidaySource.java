 ...
 * Time: 14:51
 */
public class CompositeHolidaySource implements HolidaySource {

    private List<HolidaySource> children;

    public boolean isWeekend(LocalDate date, String locale) {

        for(HolidaySource child: children) {
            if(child.isWeekend(date,locale)) {
                return true;
            }
        }
    ...
    public boolean isHoliday(LocalDate date, String locale) {
        ...
        for(HolidaySource child: children) {
            if(child.isHoliday(date,locale)) {
                return true;
            }
        }
