 ...
 * Time: 15:28
 */
public class ShortLastStubScheduleGenerator extends AbstractBaseScheduleGenerator {
    public ShortLastStubScheduleGenerator() {
        super(-1);
    }

    public ShortLastStubScheduleGenerator(int maxPeriods) {
        super(maxPeriods);
    }
    ...
    public Schedule generate(LocalDate start, LocalDate end, ReadablePeriod frequency) throws ScheduleException {
        ...
        while (holdDate.isBefore(end))
        {
            ...
            SchedulePeriod schedulePeriod = new SchedulePeriod(holdDate, nextDate, notionalStartDate, notionalEndDate);
